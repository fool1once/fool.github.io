...
def main():
    ...
    if st.button("🔄 Paraphrase Text", type="primary", use_container_width=True):
        if not text_input.strip():
            ...
            elif "Gemini Pro" in model_choice:
                with st.spinner("Gemini Pro is rewriting your text..."):
                    result_text = paraphrase_with_gemini(text_input)
                model_used = "Gemini Pro"
                score = human_score(result_text)
            elif "Ensemble" in model_choice:
                model_used, result_text, score = ensemble_paraphrase(text_input)
            ...
            with col2:
                st.metric("Human-likeness Score", f"{score}/50")
            
            # Download button
            st.download_button(
                label="📥 Download as .txt file",
                data=result_text,
                file_name=f"paraphrased_text_{datetime.now().strftime('%Y-%m-%d_%H-%M-%S')}.txt",
                mime="text/plain"
            )
